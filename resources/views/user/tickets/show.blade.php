                    ...
                    <form action="{{ route('user.tickets.rating.submit', $ticket) }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('comment') is-invalid @enderror" 
                                id="rating-comment" name="comment" rows="3" required
                                placeholder="Berikan feedback Anda tentang penanganan tiket ini..."></textarea>
                            @error('comment')
            ...
            <div class="card">
                ...
                <div class="card-body">
                    @if($ticket->status !== 'closed')
                        <form action="{{ route('user.tickets.comments.store', $ticket) }}" method="POST" class="mb-4">
                            @csrf
                            <div class="mb-3">
                    ...
                    <div class="comments-list">
                        @forelse($ticket->comments as $comment)
                            <div class="comment-item mb-3 p-3 border rounded {{ $comment->user->id === auth()->id() ? 'bg-light' : '' }}">
